...
export default function Carbon12Field({ activePublication = null }) {
    ...
    useEffect(() => {
        ...
        const drawNucleus = (cx, cy, scale, opacity, time) => {
            const nucleons = [
                [-8, -7, true], [2, -10, false], [10, -5, true],
                [-12, 2, false], [-3, 0, true], [7, 1, false],
                [14, 5, true], [-8, 9, true], [2, 10, false],
                [10, 12, false], [-1, 17, true], [-14, 14, false],
            ];
            ...
            nucleons.forEach(([x, y, proton], i) => {
                ...
                if (proton) {
                    ...
                    gradient.addColorStop(0, `rgba(112,150,200,${opacity * 1.7})`);
                    gradient.addColorStop(1, `rgba(52,92,145,${opacity})`);
                }
        ...
        const render = (time) => {
            ...
            CARBONS.forEach((carbon, carbonIndex) => {
                ...
                for (let i = 0; i < cloudCount; i++) {
                    ...
                    const x =
                        cx +
                        movingX * (1 - state.transition) +
                        targetX * state.transition;

                    const y =
                        cy +
                        movingY * (1 - state.transition) +
                        targetY * state.transition;
